...
- 문자 데이터를 날짜 데이터로 변환하는 TO_DATE 함수<br>
  TO_DATE 함수를 사용하면 문자열 데이터를 날짜 데이터로 바꿔 줄 수 있습니다.
  ```SQL
  -- 기본형식
  TO_DATE('[문자열 데이터(필수)]', '[인식될 날짜형태(필수)]')
  ```
  날짜 데이터로 변환하려면 문자열 데이터(또는 열)를 입력한 후 그 데이터를 날짜 형태로 인식시킬 형식을 지정합니다.
  ```SQL
...
특정 열 값이나 데이터 값에 따라 어떤 데이터를 반환할지 정할 때는 DECODE 함수 또는 CASE문을 사용합니다.

- DECODE 함수<br>
  DECODE 함수는 기준이 되는 데이터를 먼저 지정한 후 해당 데이터 값에 따라 다른 경과 값을 내보내는 함수입니다.
  ```SQL
  -- 기본형식
...
- CASE문<br>
  CASE문은 DECODE 함수와 마찬가지로 특정 조건에 따라 반환할 데이터를 설정할 때 사용합니다.
