...
- 집합 연산자<br>
  ...
  -- 출력 열 개수가 다를 때 (오류출력)
  결과화면: ORA-01789 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다.

  -- 출력 열의 자료형이 다를 때 (오류 출력)
  결과화면: ORA-01790 대응하는 식과 같은 데이터 유형이어야 합니다.
  
  -- 출력 열 개수와 자료형이 같을 때(출력 열의 이름은 다름)
  출력은 되지만 출력되는 열 이름은 먼저 작성한 SELECT문의 열 이름으로 표기됩니다.
  ```
...
- 연산자 우선순위<br>
  먼저 수행해야 하는 연산식을 소괄호()로 묶어 주면 연산자의 기본 우선순위와는 별개로 괄호 안의 연산식을 먼저 수행합니다.
